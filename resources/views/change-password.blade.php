@extends('layouts.app')

@section('title', 'Request Ganti Password - Mantu-Ngopi')

@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-bold text-white">Request Ganti Password</h1>
        <a href="{{ route('profile') }}"
            class="flex items-center gap-2 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 hover:text-white text-sm font-medium rounded-lg transition-colors">
            <i class="fa-solid fa-arrow-left"></i>
            Kembali
        </a>
    </div>

    @if(session('success'))
        <div
            class="mb-4 px-4 py-3 bg-green-500/10 border border-green-500/20 text-green-400 text-sm rounded-lg max-w-lg mx-auto">
            <i class="fa-solid fa-circle-check mr-2"></i>{{ session('success') }}
        </div>
    @endif

    @if($errors->any())
        <div class="mb-4 px-4 py-3 bg-red-500/10 border border-red-500/20 text-red-400 text-sm rounded-lg max-w-lg mx-auto">
            <i class="fa-solid fa-circle-exclamation mr-2"></i>
            <ul class="list-disc list-inside mt-1 space-y-1">
                @foreach($errors->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </ul>
        </div>
    @endif

    <div class="max-w-lg mx-auto space-y-4">

        {{-- Info --}}
        <div class="bg-gray-800 rounded-xl border border-yellow-500/20 p-5">
            <div class="flex items-start gap-3">
                <i class="fa-solid fa-circle-info text-yellow-400 mt-0.5"></i>
                <div>
                    <p class="text-sm font-medium text-yellow-400">Informasi</p>
                    <p class="text-xs text-gray-400 mt-1">
                        Permintaan ganti password akan dikirim ke Manager untuk disetujui.
                        Password baru akan aktif setelah Manager menyetujui permintaan.
                    </p>
                </div>
            </div>
        </div>

        {{-- Form --}}
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6">
            <form action="{{ route(auth()->user()->role . '.password.store') }}" method="POST" class="space-y-4">
                @csrf

                {{-- Password Baru --}}
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">Password Baru</label>
                    <input type="password" name="new_password"
                        class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-colors"
                        placeholder="Minimal 8 karakter">
                </div>

                {{-- Konfirmasi --}}
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">Konfirmasi Password Baru</label>
                    <input type="password" name="new_password_confirmation"
                        class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-colors"
                        placeholder="Ulangi password baru">
                </div>

                <button type="submit"
                    class="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-lg transition-colors">
                    <i class="fa-solid fa-paper-plane mr-2"></i>
                    Kirim Request
                </button>
            </form>
        </div>

        {{-- History Request --}}
        @if(isset($requests) && $requests->count() > 0)
            <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-700">
                    <h2 class="text-sm font-semibold text-gray-300">Histori Request</h2>
                </div>
                <div class="divide-y divide-gray-700">
                    @foreach($requests as $req)
                        <div class="flex items-center justify-between px-5 py-3">
                            <span class="text-xs text-gray-400">{{ $req->created_at->format('d M Y, H:i') }}</span>
                            @php
                                $reqColor = match ($req->status) {
                                    'pending' => 'bg-yellow-500/10 text-yellow-400',
                                    'approved' => 'bg-green-500/10 text-green-400',
                                    'rejected' => 'bg-red-500/10 text-red-400',
                                    default => 'bg-gray-500/10 text-gray-400',
                                };
                            @endphp
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium capitalize {{ $reqColor }}">
                                {{ $req->status }}
                            </span>
                        </div>
                    @endforeach
                </div>
            </div>
        @endif

    </div>
@endsection